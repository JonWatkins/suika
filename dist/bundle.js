...
const h = (t) => t !== null && typeof t == "object",
...
const q = (t) => typeof t == "string",
  L = (t) => new Promise((n) => setTimeout(n, t));
function g(t, n) {
  const e = /* @__PURE__ */ new Map(),
    s = t.split(",");
  for (let i = 0; i < s.length; i++) e.set(s[i], !0);
...
const j = g(
  ...
  b = (t, n) =>
    t && n && h(t) && h(n) && typeof t == typeof n
      ? Object.keys(t).length === Object.keys(n).length &&
        Object.keys(t).every((e) => b(t[e], n[e]))
      : t === n,
...
function m(t, n, e = []) {
  ...
  for (const c in t) t[c] = m(t[c], n, e.concat(c));
  const i = (c, u) => {
    ...
    r = (c, u, f, x) => {
      const C = Reflect.set(c, u, m(f, n), x);
      return (
        typeof n == "function" &&
          n({
            path: s(u),
...
const H = () => "fragment",
  ...
  o = (t) => {
    if (t.kind === "text") return document.createTextNode(t.value);
    if (t.kind === "function") return o(t.component(t.attrs, t.children));
    if (t.kind === "component") {
      if (t.instance) {
      ...
      const e = t.instance._initVnode(t.attrs),
        s = o(e);
      return t.instance._notifyMounted(s), s;
    }
    const n = document.createElement(t.tag);
    for (const e in t.attrs) n[e] = t.attrs[e];
    return (
  ...
  nt = (t, n) => {
    if (!l(t) || !O(t)) throw new Error("Must pass a component to mount");
...
function k(t, n) {
  ...
  if (t.kind === "function" && n.kind === "function")
    return k(t, n.component(n.attrs, n.children));
  if (
    t.kind === "component" &&
    n.kind === "component" &&
    ...
    return (
      ...
      b(t.attrs, n.attrs)
        ? (i) => i
        : (n.instance._setAttrs(n.attrs), n.instance._getDiff())
    );
  if (n.kind === "component")
    return (
      (n.instance = new n.component()),
      n.instance._initState(),
      n.instance._initVnode(n.attrs),
      (i) => {
        const r = o(n);
  ...
  const e = Q(t.attrs, n.attrs),
    s = X(t.children, n.children);
  return (i) => (e(i), s(i), i);
}
function Q(t = {}, n = {}) {
  const e = {
    remove: Object.keys(t || {}).filter((s) => !l(n[s])),
    set: Object.keys(n || {})
      .filter((s) => t[s] !== n[s] && l(n[s]))
      .reduce((s, i) => ({ ...s, [i]: n[i] }), {}),
  };
  return (s) => {
    for (const i of e.remove) s.removeAttribute(i);
    for (const i in e.set) s[i] = e.set[i];
  };
}
function X(t, n) {
  const e = [],
  ...
  for (const i of n.slice(t.length)) s.push((r) => (r.appendChild(o(i)), r));
  return (i) => {
    if (i) {
      for (const [r, d] of Z(e, i.childNodes)) r(d);
      for (const r of s) r(i);
    }
    return i;
  };
}
function _(t, n) {
  ...
  for (; e--; ) {
    ...
    if (s.kind === "component") {
      const i = Y(s, n);
      i && _(s.instance._vNode.children, i.instance._vNode.children),
        i || s.instance._unmount();
    }
  }
}
function Y(t, n) {
  return n.find((e) => t.component === e.component);
}
const Z = (t, n) => {
  const e = [];
  for (let s = 0; s < Math.min(t.length, n.length); s++) e.push([t[s], n[s]]);
  return e;
};
let $ = 0;
class w {
...
export { w as Component, H as Fragment, tt as h, nt as mount, V as utils };
